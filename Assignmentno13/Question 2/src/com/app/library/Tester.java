...
public class Tester 
{
	...
	{
		try(FileOutputStream fout=new FileOutputStream("Library1.txt"))
		{
			try(ObjectOutputStream oout=new ObjectOutputStream(fout))
			{
					for(int i=0;i<books.size();i++)
					{
						oout.writeObject(books.get(i));
					}
			}
		}
		catch(Exception e)
	...
	public static ArrayList<Library> read()
	{
		...
		ArrayList<Library> n=new ArrayList<>();
		try(FileInputStream fout=new FileInputStream("Library1.txt"))
		{
			try(ObjectInputStream ooin=new ObjectInputStream(fout))
			{
				 while(true)
				 {
					Library books =(Library)ooin.readObject();
					 n.add(books);
				 }
